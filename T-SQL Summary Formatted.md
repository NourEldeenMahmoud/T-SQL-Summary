...
## 🌟 Introduction
T-SQL (Transact-SQL) is a powerful programming language used in Microsoft SQL Server for managing and processing data. It includes tools like **variables**, **control-of-flow structures**, **stored procedures**, **triggers**, **functions**, **cursors**, **common table expressions (CTEs)**, and **ranking functions** to enhance performance and readability.
...
```mermaid
graph TD
    A[Trigger Event (e.g., INSERT)] --> B[Check Condition]
...
###### Example 1: Non-Recursive CTE - Calculate Average Grades
```
...
SELECT t.Name, a.AvgGrade
FROM Teachers t
JOIN AverageGrades a ON t.Subject = a.Subject;
```
- **Explanation**: The CTE `AverageGrades` computes averages, which are then joined with the `Teachers` table.
...
ORDER BY Level, Name;
```
- **Explanation**: The recursive CTE starts with top-level employees and iteratively adds subordinates.

##### Notes
- **Performance**: Can improve readability but may not always optimize performance; use indexes wisely.
- **Scope**: Limited to the query where defined; not reusable across multiple queries.
- **Recursion Limit**: Recursive CTEs have a default depth limit (100) to prevent infinite loops.

##### 📊 CTE Workflow
```mermaid
graph TD
    A[Define CTE with WITH] --> B[Execute Main Query]
    B --> C[Use CTE Result]
    C --> D[Complete]
```

##### 💡 Tips
- Use meaningful CTE names to enhance clarity.
- Avoid overuse in large datasets; consider temporary tables for complex cases.
- Test recursive CTEs with a `MAXRECURSION 0` option to debug infinite loops (set to unlimited).
